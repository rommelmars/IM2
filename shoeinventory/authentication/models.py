...
class Sale(models.Model):
    ...
    shoe_image = models.ImageField(upload_to='images/', null=True, blank=True)  # Store the shoe image
    shoe_category = models.ForeignKey(Category, on_delete=models.SET_NULL, null=True, blank=True)  # Store the shoe category
    quantity_sold = models.PositiveIntegerField()
    date = models.DateTimeField(default=timezone.now)
    total_amount = models.DecimalField(max_digits=10, decimal_places=2, blank=True)

    def save(self, *args, **kwargs):
        # Store the shoe name, image, and category if the shoe is available
        if self.shoe:
            self.shoe_name = self.shoe.name
            self.shoe_image = self.shoe.image
            self.shoe_category = self.shoe.category
